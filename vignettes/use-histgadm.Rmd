...
head(vn_history) # list of event in admin1 history in vietnam
```

Now, we can use the function `hist_map` to create "historical" map of Vietnam in between 2000 and 2019:
```{r}
a <- hist_map("Vietnam", vn_admin1, vn_history, from = 2000, to = 2019)
a
```
...
histgadm::map_data(tmp, "Cambodia", dictionary::kh_admin1, dictionary::kh_history, from = "1980", to = "2018", save = TRUE)
```
